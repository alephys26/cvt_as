...
class BuildingCreation:
    ...
    def create_hostel(self):
        ...
        for floor in range(1, 4):  
            ...
            self.cooridate_building[floor_node]=self.coordinate
            self.G.add_edge(previous_node, floor_node, weight=1)  
            previous_node = floor_node  

            for room in range(1, 4):  
                room_node = f'Room {floor}0{room}'
                self.cooridate_building[room_node]=(self.coordinate[0]+arr1[room-1],self.coordinate[1]+arr2[room-1],floor)
    ...
    def create_department(self):
        ...
        for floor in range(1, 3):  
            floor_node = f'Floor {floor}'
            self.cooridate_building[floor_node]=(self.coordinate[0],self.coordinate[1],floor)
            self.G.add_edge(previous_node, floor_node, weight=1)  
            previous_node = floor_node  

            for room in range(1, 3):  
                room_node = f'Room {floor}0{room}'
                self.cooridate_building[room_node]=(self.coordinate+arr1[room-1],self.coordinate+arr2[room-1],floor)
                self.G.add_edge(floor_node, room_node, weight=1)  

    def create_director(self):
        self.G.add_node(self.building_name)
        floor_node = f'Floor 1'
        self.cooridate_building[floor_node]=(self.coordinate[0],self.coordinate[1],1)
        self.G.add_edge(self.building_name, floor_node, weight=1)  
        room_node = 'Room 101'
        self.cooridate_building[floor_node]=(self.coordinate[0]+0,self.coordinate[1]+0.1,1)
        self.G.add_edge(floor_node, room_node, weight=1)  
